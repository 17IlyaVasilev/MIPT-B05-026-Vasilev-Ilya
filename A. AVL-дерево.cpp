...
int main() {
	...
	while (cin >> s >> x) {
		...
		if (s == "insert") tree.insert(root, x);
		else if (s == "delete") tree.remove(root, x);
		else if (s == "exists") cout << (tree.find(root, x) ? "true" : "false") << endl;
		else if (s == "next") {

			node* tmp = tree.find_next(root, x);
			...
			node* tmp = tree.find_prev(root, x);
			if (tmp == nullptr || tmp->key == INF)cout << "none" << endl;
			else cout << tmp->key << endl;
		}
